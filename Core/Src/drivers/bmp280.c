...
void Bmp280_WriteReg(uint8_t reg, uint8_t value, uint8_t length) {
    ...
    i2cWrite(BMP280_ADDR, reg, &value, length,I2C1);
}
...
void bmp280_init()
{
	...
	i2cRead(BMP280_ADDR, CHIP_ID, 1, &whoAmIBuffer,I2C1);
	...
	i2cRead(BMP280_ADDR, CTRL_MEAS, 1, &ctrl_meas_adjustment,I2C1);
...
void bmp280_read_calibration(void)
{
    ...
    i2cRead(BMP280_ADDR, CALIB_START, 26, buffer,I2C1);
...
int32_t bmp280_read_raw_temp()
{
    ...
    i2cRead(BMP280_ADDR, TEMP_MSB, 3, buffer,I2C1);
...
int32_t bmp280_read_raw_press()
{
    ...
    i2cRead(BMP280_ADDR, PRESS_MSB, 3, buffer,I2C1);
