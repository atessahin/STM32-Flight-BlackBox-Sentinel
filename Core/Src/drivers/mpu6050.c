...
void MPU_WriteReg(uint8_t reg, uint8_t value,uint8_t length) {
    ...
    i2cWrite(MPU6050_ADDR, reg, &value, length,I2C1);
}
...
void mpu6050_init(void)
{
	...
	i2cRead(MPU6050_ADDR, WHO_AM_I, 1, &whoAmIBuffer,I2C1);
...
void mpu6050_calibrate_gyro()
{
	...
	for (int i = 0; i < 200;  i++) // Sample 200 times
	{
		 ...
		 i2cRead(MPU6050_ADDR,GYRO_XOUT_H, 6, calibrateBuff2,I2C1);
...
void mpu6050_read_accel(int16_t *ax, int16_t *ay, int16_t *az)
{
    ...
    i2cRead(MPU6050_ADDR, ACCEL_XOUT_H, 6, collabBuff,I2C1);
...
void mpu6050_read_gyro(int16_t *gx, int16_t *gy, int16_t *gz)
{
	 ...
	 i2cRead(MPU6050_ADDR,GYRO_XOUT_H, 6, collabBuff2,I2C1);
