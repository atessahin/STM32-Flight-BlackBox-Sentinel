...
sd_error sd_card_receive_byte(SPI_HandleTypeDef *const hspi, uint8_t* data)
{
    *data = spiRxByte();
    return SD_OK;
}
...
)
{
  ...
  uint32_t captured_tick =millis();

  do
  {
    if ((millis() - captured_tick) > SD_TRANSMISSION_TIMEOUT)
      return SD_TIMEOUT;
...
)
{
  ...
  CS_LOW;
  sd_error status = sd_card_transmit_bytes(hspi, (uint8_t*)&cmd_send_csd, sizeof(cmd_send_csd));
