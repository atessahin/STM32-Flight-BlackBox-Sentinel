...
)
{
  ...
  CS_LOW;
  sd_error status = sd_card_transmit_bytes(hspi, (uint8_t*)&cmd_read_single_block, sizeof(cmd_read_single_block));
...
)
{
  ...
  CS_LOW;
  sd_error status = sd_card_transmit_bytes(hspi, (uint8_t*)&cmd_read_multiple_block, sizeof(cmd_read_multiple_block));
  status |= sd_card_receive_cmd_response(hspi, &r1, 1);
  ...
  status = sd_card_transmit_bytes(hspi, (uint8_t*)&cmd_stop_transmission, sizeof(cmd_stop_transmission));
