...
#include "dma_bus.h"
#include"delay.h"
typedef SPI_TypeDef SPI_HandleTypeDef;
#define CS_HIGH GPIOB->BSRR = (1 << 12);
#define CS_LOW  GPIOB->BSRR = (1 << (12+16));
// Defines -------------------------------------------------------------------
...
#define SEND_CMD(p_hspi, cmd, response, status) \
  CS_LOW; \
  (status) |= sd_card_transmit_bytes((p_hspi), (uint8_t*)&(cmd), 6); \
  (status) |= sd_card_receive_cmd_response((p_hspi), (uint8_t*)&(response), sizeof(response)); \
  CS_HIGH
