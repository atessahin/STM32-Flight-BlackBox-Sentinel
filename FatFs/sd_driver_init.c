...
static sd_error sd_card_enter_spi_mode(SPI_HandleTypeDef *const hspi)
{
  ...
  CS_LOW;//cs low says hey wake up i will talk to you if cspin not low while sending cmd0 sd card never be spi mode

  //send CMD0(GO_IDLE_STATE) byte to byte
  uint8_t *cmd_ptr = (uint8_t*)&cmd_go_idle_state;
  for (int i = 0; i < 6; i++) { // command always should be 6 byte (read/write/reset)it doesnt matter
      spiTxByte(cmd_ptr[i]);
  }

  //wait sd_card response with timeout
  uint32_t tickstart = millis();
  do {
      r1 = spiRxByte(); //direct read hardware
      if ((millis() - tickstart) > 500) {
          CS_HIGH;
          return SD_TIMEOUT;
      }
      //loop until the 8th bit (MSB) of r1 becomes 0 (that means a valid response has arrived) and
      //until r1 is not 0x01 (R1_IN_IDLE_STATE)
      //(While 0xFF which is 255 is received, this loop keeps waiting)
      } while ((r1 & 0x80) != 0 || r1 != R1_IN_IDLE_STATE);
